...
class DetailController extends Controller
{
    ...
    public function postAdd(DetailRequest $request){
        ...
        $folder_create = convert_vi_to_en(strip_tags($request->input('tittle')));
        $folder = 'storage/uploads/detail_images/' .$folder_create;
    ...
    public function postEdit($id, Request $request){
        ...
        $rules = [
            'tittle' => 'required|unique:details,tittle,'.$id
        ];

        $messages = [
            'tittle.required' => 'tittle can not be empty.',
            'tittle.unique' => 'tittle is exist.'
        ];

        $validator = Validator::make($request->all(), $rules, $messages);
		$folder_create = convert_vi_to_en(strip_tags($request->input('tittle')));

        $current_image = ('storage/uploads/detail_images/'.$folder_create.'/'.$detail["images"]);
        ...
        if(!empty($request->file('images'))){
            ...
            $detail->save();
            $request->file('images')->move('storage/uploads/detail_images/'.$folder_create.'/',$file_name);
